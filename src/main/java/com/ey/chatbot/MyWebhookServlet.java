...
// Fulfillment url in API AI = https://beta-007-dot-poc-iot.appspot.com/webhook
...
public class MyWebhookServlet extends AIWebhookServlet  { 
	...
	protected void doWebhook(AIWebhookRequest input, Fulfillment output) {
		log.info("inside webhook handler");
		...
		//Mapping response with request type
		switch (action) {
		...
		case "query":
			...
			log.info("topic : "+ topic +"  law_scope : "+law_scope);
	...
	protected Fulfillment getQueryResponse(String topic , String law_scope , Fulfillment output){
		String response = "" ;
		log.info("method getQueryResponse ");
		if (law_scope.equals("FEDERAL")) {
			try {
				...
				log.severe("Exception in getQueryResponse :"+e);
			}
		}
	...
	protected Fulfillment getStateActionResponse(String topic , String state , Fulfillment output){
		...
		try {
			...
			log.severe("Exception in getStateActionResponse :"+e);
